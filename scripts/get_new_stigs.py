...
def get_stig_zip(output_name, stig_sys="ASD", stig_ver="V6R3", allowed_dirs=None):
    """
    ...
    # Validate input parameters to prevent URL manipulation
    ...
    try:
        # Download the ZIP file with timeout and size limits
...
def extract_zip(file_path, allowed_dirs=None):
    """
    Securely extracts the zip file, preventing path traversal attacks
    """
    if allowed_dirs is None:
        allowed_dirs = get_default_allowed_dirs()
