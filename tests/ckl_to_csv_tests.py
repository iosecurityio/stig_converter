...
def convert_to_csv(checklists=[]):
    ...
    fieldnames = ['HOST_NAME', 'HOST_IP', 'Vuln_Num', 'Severity', 'STATUS', 'Group_Title', 'Rule_ID', 'Rule_Ver', 'Rule_Title', 'Vuln_discuss', 'Check_Content', 'Fix_Text', 'FINDING_DETAILS', 'COMMENTS']
    ...
    for checklist in checklists:
        ...
        with open(new_csv, 'w', newline='') as csvfile:
            ...
            # Parse the ASSET node of the ckl for asset details
            ...
            for vuln in root.iter('VULN'):
                for stig_data in vuln.findall('./STIG_DATA'):
                    if (stig_data.find('VULN_ATTRIBUTE').text in ['Vuln_Num', 'Severity', 'Group_Title', 'Rule_ID', 'Rule_Ver', 'Rule_Title', 'Vuln_discuss', 'Check_Content', 'Fix_Text']):
...
def main():
    ...
    #2. Get list of checklists that exist and print them
